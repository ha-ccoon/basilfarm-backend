...
export const getActuatorData = async (req, res) => {
    try {
      const [rows] = await connection.execute('SELECT * FROM actuator_config');
      res.json(rows);
    } catch (err) {
      console.error(err);
...
export const getActuatorDataByDeviceId = async (req, res) => {
    ...
    try {
      const [rows] = await connection.execute('SELECT * FROM actuator_config WHERE device_id = ?', [device_id]);
      res.json(rows);
    } catch (err) {
      console.error(err);
      res.status(500).send('Internal Server Error');
