...
const saveAutoStatus = async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).send('명령 저장에 실패하였습니다.');
  }
};
...
// 자동화 ON/OFF 구분 후 시행
const autoManagement = async (req, res, next) => {
  let intervalId;
  const { device_id } = req.params;
  const { status, target_temp, target_moisture, target_light } = req.body;

  if (status === 1) {
    intervalId = startAutoManagement(device_id, target_temp, target_moisture, target_light);
...
export { saveAutoStatus, autoManagement, currentAutoManagementStatus };